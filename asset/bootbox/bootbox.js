...
(function (root, factory) {
    ...
    exports.form = function() {
        ...
        if(options.message) {
            var tmps = '<div class="col-md-12 m-b20">'+options.message+'</div>';
            options.message = form.prepend(tmps);
        }else{
            options.message = form;
        }
        ...
        options.buttons.confirm.callback = function() {
            ...
            each(options.fields, function(field,opts) {
                ...
                if(opts.id)  inputs[field].attr('id',opts.id);
                if(opts.class)  inputs[field].addClass(opts.class);
                if(opts.readonly)  inputs[field].attr('readonly','readonly');
            });
        ...
        each(options.fields,function(field, opts) {
            ...
            switch (opts.type) {
                ...
                case "select":
                    ...
                    each(inputOptions, function(_, option,opts) {
        ...
        if(options.message2) {
            var tmps = '<div class="col-md-12 m-b20">'+options.message2+'</div>';
            options.message = form.append(tmps);
        }else{
            options.message = form;
        }
